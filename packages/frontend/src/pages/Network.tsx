...
import { GelElectrophoresis, GelMultiCoin } from "../components/GelElectrophoresis";
import { NETWORK_ENZYMES } from "@zcoin/core";
  ...
  return (
    <div className="page">
      ...
      <p className="text-muted mb-2" style={{ maxWidth: 700 }}>
        The zBioCoin network is a living organism. Its DNA stores every coin ever minted.
...
function OverviewTab({ stats, dnaData, dnaLoading, networkRflp }: { stats: NetworkStats; dnaData: NetworkDnaAnalysis | null; dnaLoading: boolean; networkRflp: RFLPFingerprint | null }) {
  return (
    <div className="net-content">
      ...
      {networkRflp && dnaData && dnaData.coins.length > 0 && (
        <div className="card" style={{ marginTop: "1.5rem" }}>
          <h2>DNA Fingerprinting &mdash; Gel Electrophoresis</h2>
          <p className="text-muted text-sm mb-2">
            Like a real paternity test: the network's restriction enzymes ({NETWORK_ENZYMES.map((e) => e.name).join(", ")}) 
            cut both the network DNA and each coin's gene. Matching bands prove the coin is a legitimate child of this network.
          </p>
          <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap", alignItems: "flex-start" }}>
            <div>
              <GelMultiCoin
                networkFragments={networkRflp.fragments.slice(0, 30)}
                coins={dnaData.coins.slice(0, 6).map((c, i) => ({
                  label: `#${i + 1}`,
                  fragments: c.rflpFragments && c.rflpFragments.length > 0
                    ? c.rflpFragments
                    : [c.length * 3],
                  valid: (c.rflpMarkerCount ?? 0) >= 2,
                }))}
                height={280}
              />
            </div>
            <div style={{ flex: 1, minWidth: 240 }}>
              <div className="gel-legend">
                <div className="gel-legend-item">
                  ...
                  <span>Network DNA bands (parent)</span>
                </div>
                <div className="gel-legend-item">
                  <span className="gel-legend-swatch" style={{ background: "#22c55e" }} />
                  <span>Coin bands (child) &mdash; matching = valid</span>
                </div>
                <div className="gel-legend-item">
                  <span className="gel-legend-swatch" style={{ background: "#ef4444" }} />
                  <span>Foreign coin bands &mdash; no match = forgery</span>
                </div>
              </div>
              <p className="text-xs text-muted" style={{ marginTop: "0.75rem", lineHeight: 1.7 }}>
                Each lane shows the fragment pattern after restriction enzyme digestion. 
                The network inserts markers at secret positions (derived from its private key) 
                during coin signing. These markers create the characteristic band pattern that 
                proves the coin was signed by this specific network &mdash; just like how RFLP 
                is used in forensic DNA identification.
              </p>
      ...
      <div className="synthesis-grid">
        <div className="synth-card synth-coins">
          ...
          <div className="synth-desc">
            ...
            Each one is a mined and signed zBioCoin.
          </div>
        </div>
            ...
            <div key={coin.serialHash} className={`coin-card ${isExpanded ? "expanded" : ""}`}>
              ...
              {isExpanded && (
                <div className="coin-details">
                  ...
                  {networkRflp && coin.rflpFragments && (
                    <div className="field">
                      <label className="label">RFLP Gel Electrophoresis</label>
                      <p className="text-xs text-muted" style={{ marginBottom: "0.5rem" }}>
                        Matching bands between network (purple) and coin (green) prove parentage.
                      </p>
                      <GelMultiCoin
                        networkFragments={networkRflp.fragments.slice(0, 20)}
                        coins={[{
                          label: `#${i + 1}`,
                          fragments: coin.rflpFragments,
                          valid: (coin.rflpMarkerCount ?? 0) >= 2,
                        }]}
                        height={200}
                      />
                    </div>
